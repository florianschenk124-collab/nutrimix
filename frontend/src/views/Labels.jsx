...
export default function Labels(){const{t,td,sd}=useI18n();const[recipes,setRecipes]=useState([]);const[wp,setWp]=useState([]);const[params,setParams]=useState({recipe_name:'',water_profile_name:'Osmosewasser',concentrate_factor:100,volume_l:10,note:''});const[result,setResult]=useState(null);const[loading,setLoading]=useState(false)
...
<LC t={t} sd={sd} tank="A" color="var(--tank-a)" rn={params.recipe_name} water={params.water_profile_name} factor={params.concentrate_factor} tv={params.volume_l} date={today} note={params.note} salts={result.tank_a} ec={result.ec_ionic} ph={recipe?`${recipe.ph_min}–${recipe.ph_max}`:'–'} dA={result.dose_ratio_a} dB={result.dose_ratio_b}/>
<LC t={t} sd={sd} tank="B" color="var(--tank-b)" rn={params.recipe_name} water={params.water_profile_name} factor={params.concentrate_factor} tv={params.volume_l} date={today} note={params.note} salts={result.tank_b} ec={result.ec_ionic} ph={recipe?`${recipe.ph_min}–${recipe.ph_max}`:'–'} dA={result.dose_ratio_a} dB={result.dose_ratio_b}/></div>)}
...
function LC({t,sd,tank,color,rn,water,factor,tv,date,note,salts,ec,ph,dA,dB}){const fmt=(v,d=1)=>Number(v).toFixed(d);const ml=1000/factor;const oT=tank==='A'?'B':'A'
...
<tbody>{salts.map(s=><tr key={s.salt_formula}><td style={{fontSize:12}}>{sd(s.salt_name,s.salt_formula)}</td><td className="num" style={{fontWeight:700}}>{fmt(s.g_concentrate*(tv/(1000/factor)),1)}</td></tr>)}</tbody></table>
