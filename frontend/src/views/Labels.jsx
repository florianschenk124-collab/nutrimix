...
export default function Labels(){const{t}=useI18n();const[recipes,setRecipes]=useState([]);const[wp,setWp]=useState([]);const[params,setParams]=useState({recipe_name:'',water_profile_name:'Osmosewasser',concentrate_factor:100,volume_l:10,note:''});const[result,setResult]=useState(null);const[loading,setLoading]=useState(false)
...
const today=new Date().toLocaleDateString('de-DE');const fmt=(v,d=1)=>Number(v).toFixed(d);const recipe=recipes.find(r=>r.name===params.recipe_name)
...
<tbody>{salts.map(s=><tr key={s.salt_formula}><td style={{fontSize:12}}>{s.salt_name}</td><td className="num" style={{fontWeight:700}}>{fmt(s.g_concentrate*(tv/(1000/factor)),1)}</td></tr>)}</tbody></table>
