...
<div className="form-group"><label className="form-label">{t('c.water_profile')}</label><select className="form-select" value={cw} onChange={e=>setCw(e.target.value)}>{wp.map(w=><option key={w.name} value={w.name}>{w.name}</option>)}</select></div></div>
...
<tbody>{[...res.tank_a,...res.tank_b].filter(s=>s.g_total>0).map(s=>(<tr key={s.salt_formula}><td style={{fontSize:12}}>{s.salt_name}</td>
