...
export default function Compatibility(){const{t,td,sd}=useI18n();const[data,setData]=useState(null);const[loading,setLoading]=useState(true);const[filter,setFilter]=useState('all')
...
<td style={{padding:'7px 14px',fontSize:12}}>{sd(c.salt_a,c.formula_a||c.salt_a)}</td><td style={{padding:'7px 14px',fontSize:12}}>{sd(c.salt_b,c.formula_b||c.salt_b)}</td>
